  ...
  const faqs = [
    {
      ...
      answer: 'Стоимость индивидуальной консультации — 5000 рублей за 60 минут. Для программы из 8 сессий предусмотрены пакетные предложения с выгодой до 15%. Первая диагностическая сессия может проводиться по специальной цене.'
    },
    {
      question: 'Работаете ли вы онлайн?',
      answer: 'Да, я провожу консультации как очно в Москве, так и онлайн через удобные видеосвязи (Zoom, Telegram). Эффективность онлайн-консультаций доказана исследованиями и не уступает очным встречам.'
    },
    {
    ...
    },
    {
      ...
      answer: 'Нет, для консультации психолога направление не требуется. Вы можете записаться самостоятельно. Если в процессе работы я увижу необходимость медицинской поддержки (например, при клинической депрессии), порекомендую обратиться к психиатру.'
    },
    {
          ...
          <Accordion type="single" collapsible className="space-y-4">
            {faqs.map((faq, index) => (
              <AccordionItem 
                key={index} 
                value={`item-${index}`}
                className="bg-white border-2 border-primary/10 rounded-lg px-6 hover:border-primary/30 transition-colors"
              >
