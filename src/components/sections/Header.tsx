        ...
        <nav className="hidden md:flex gap-6">
          {navLinks.map((link) => (
            <a
              key={link.href}
              href={link.href}
              className={`font-bold text-foreground/70 hover:text-primary transition-colors ${link.bold ? 'font-extrabold' : ''}`}
            >
              {link.label}
            </a>
          ))}
            ...
            <nav className="flex flex-col gap-4 mt-8">
              {navLinks.map((link) => (
                <a
                  ...
                  className={`text-lg font-bold text-foreground/70 hover:text-primary transition-colors py-2 ${link.bold ? 'font-extrabold' : ''}`}
                >
                  {link.label}
                </a>
              ))}
